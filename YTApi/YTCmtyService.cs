...
public class YTCmtyService
{
    ...
    public static InitialData GetInitialData(
        ...
        string cookies)
    {
        return ApiFunction.GetInitialData(
            httpClient: httpClient,
            channelID: channelID,
            cookies: cookies);
    }
        ...
        bool getWholePosts = false)
    {
        ...
        InitialData initialData = ApiFunction.GetInitialData(
            httpClient: httpClient,
            channelID: channelID,
            cookies: cookies);
        ...
        if (latestPosts.Count > 0)
        {
            ...
            if (getWholePosts)
            {
                ...
                {
                    List<PostData> oldertPosts = GetEarlierPosts(
                        httpClient: httpClient,
                        ytConfig: configData,
                        cookies: cookies,
                        referer: referer);

                    if (oldertPosts.Count > 0)
                    {
        ...
        if (contentsArray != null)
        {
            ...
            foreach (JsonElement content in contentsArray)
            {
                JsonElement? itemSectionRendererContents =
                    JsonParser.GetItemSectionRendererContents(jsonElement: content);
                JsonElement.ArrayEnumerator? itemSectionRendererContentsArray =
                    itemSectionRendererContents?.GetArrayEnumerator();
                ...
                if (itemSectionRendererContentsArray != null)
                {
                    ...
                    if (backstagePostThreadRenderers != null)
                    {
                        ...
                        {
                            JsonElement? backstagePostRenderer = JsonParser
                                .GetBackstagePostRenderer(jsonElement: backstagePostThreadRenderer);
    ...
    /// <returns>List&lt;PostData&gt;</returns>
    public static List<PostData> GetEarlierPosts(
        HttpClient httpClient,
        YTConfig? ytConfig,
        string cookies = "",
        string referer = "")
    {
        ...
        JsonElement jsonElement = ApiFunction.GetJsonElement(
            httpClient: httpClient,
            ytConfig: ytConfig,
            cookies: cookies,
            referer: referer);
