...
- [YouTube 訂閱者計數器工具](#九youtube-訂閱者計數器工具)
...
1. 先在右側的短片清單中新增內容；或使用 [網路資源](#二網路資源) 取得內容。
2. 點選`短片播放器`頁籤。
3. 點選`播放`按鈕，即可播放短片。
...
- 點選`停止`按鈕，即可停止播放短片。
- 點選`短片播放器`開關，以切換成`時間標記編輯器`模式；點選`時間標記編輯器`開關，以切換成`短片播放器`模式。
...
3. 勾選`匯出成 *.jsonc`以匯出 [YoutubeClipPlaylist/Playlists](https://github.com/YoutubeClipPlaylist/Playlists) 所使用的格式。
4. 勾選`檢查網址`以啟用網址檢查功能。
   - 會排除拜年紀這類的影片。
5. 點選`產生短片清單檔案`按鈕即可。
...
1. 點選`查詢使用者代理字串`按鈕，即會開啟您預設的網頁瀏覽器，並透過 [WhatIsMyBrowser.com](https://www.whatismybrowser.com/detect/what-is-my-user-agent/) 網站來查詢該網頁瀏覽器的使用者代理字串。
...
本應用程式的短片分割、[燒錄字幕檔](#六燒錄字幕檔)以及[分割影片](#七分割影片)等功能皆是透過 FFmpeg 達成，此處提供簡易的 FFmpeg 選項設定功能。
